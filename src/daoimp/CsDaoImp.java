...
public class CsDaoImp implements CsDao {
	...
	public void addEmp(ECs cs) {
		System.out.println("-----ADD class: "+cs.getClasss()+" grade: "+cs.getGrade());
		session.save(cs);// 添加数据
		tr.commit();// 提交事务
	}

	public List<ECs> getAll(int pageNo, int pageSize) {
		session.beginTransaction();
		List<ECs> list = new ArrayList<ECs>();
		// 把查询结果放入list集合中，查询结果是根据页数来显示记录数
		list = session.createQuery("from hibernate.ECs").setFirstResult((pageNo - 1) * pageSize)// 每页显示的起始数据，pageNo表示页数
	...
	public int maxPage() {// 求最大页数的方法
		...
		session.beginTransaction();
		// 获取总记录数
		count = (Integer) session.createQuery("select count(*) from hibernate.ECs").uniqueResult();
		maxpage = (count + 4) / 5;// 这里的5是每页显示的条数，4是每页显示条数减1
		return maxpage;
	}

	public List<ECs> queryAllEmp() {
		List<ECs> list = session.createQuery("from hibernate.ECs").list();// 查询全部
		tr.commit();// 提交事务
	...
	public ECs queryAdById(int csid) {
		ECs ad = new ECs();
		ad = (ECs) session.load(ECs.class, csid);//根据id查询
		return ad;
	}

	@Override
	public void updateAd(ECs par) {
	...
	public void deleteAd(int csid) {
		// TODO Auto-generated method stub
		Query query = (Query) session.createQuery("delete ECs where csid=:id");
