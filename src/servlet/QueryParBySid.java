...
public class QueryParBySid extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		list = dao.queryParBySid(sid);// 调用实现类中根据id查询的方法
		request.setAttribute("list", list);// 把获得的对象放入request对象中
		// 转发到现实单条记录的页面
		request.getRequestDispatcher("frnt/teacher_class/QueryParBySid.jsp").forward(request, response);
		out.flush();
		out.close();
	}
}